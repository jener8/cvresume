...
export default function Home() {
  ...
  const persistResumeVersion = useCallback(async () => {
    const s = resumePersistRef.current
    if (s.currentVersionIndex === null) return
    const i = s.currentVersionIndex
    setIsUpdatingVersion(true)
    try {
      const updatedVersions = [...s.versions]
      updatedVersions[i] = {
        ...updatedVersions[i],
        ...
        timestamp: Date.now(),
        jobAdvertSource: s.contactInfo.jobAdvertSource || "",
      }
      setVersions(updatedVersions)
      await saveResumeVersions(updatedVersions)
  ...
  const resumeAutosaveSnapshot = useMemo(() => {
    const jd = currentVersion?.jobDescription ?? ""
  ...
  const resumeAutosaveResetKey = `${currentFolderId ?? "nofolder"}:${currentVersion?.id ?? "no-version"}`
  ...
  const resumeAutosaveStatus = useDebouncedAutosave({
    ...
    enabled: view === "resume" && currentVersionIndex !== null,
    debounceMs: 1000,
  })
  ...
  const finishBackToDashboard = () => {
    setShowBackConfirmDialog(false)
    setHasUnsavedChanges(false)
    setView("dashboard")
  }

  const handleBackConfirmSave = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault()
    ...
    try {
      ...
      finishBackToDashboard()
    } catch (err) {
      console.error("[v0] Back confirm save failed:", err)
      toast({
  ...
  const handleCreateCoverLetter = async () => {
    ...
    setStandaloneCoverLetterMode("wizard")
    setView("coverLetter")
  }
  ...
  const handleOpenCoverLetterEditorFromResume = () => {
    if (!currentResumeCoverLetter) return
    setSelectedCoverLetterId(currentResumeCoverLetter.id)
    setStandaloneCoverLetterMode("editor")
    setView("coverLetter")
  }
  ...
  const handleResumeJobDescriptionChange = useCallback(
    (newJobDescription: string) => {
      if (currentVersionIndex === null) {
        ...
        clearTimeout(pendingJobDescriptionTimerRef.current)
        pendingJobDescriptionTimerRef.current = setTimeout(() => {
  ...
  const currentResumeCoverLetter = coverLetters.find((l) => l.name === `Cover Letter - ${currentVersion?.name || "New Resume"}`)
    ...
    return (
      <Dashboard
        ...
        onOpenCoverLetterWizard={(jobId) => {
          setSelectedJobId(jobId)
          setJobCoverLetterMode("wizard")
          setView("jobCoverLetter")
        }}
        onOpenCoverLetterEditor={(jobId) => {
          setSelectedJobId(jobId)
          setJobCoverLetterMode("editor")
          setView("jobCoverLetter")
        }}
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <AlertDialog
            ...
            onOpenChange={(open) => {
              if (!open && backConfirmSaving) return
              setShowBackConfirmDialog(open)
            }}
          >
            <AlertDialogContent>
              <AlertDialogHeader>
              ...
              <AlertDialogFooter>
                <AlertDialogCancel onClick={handleBackConfirmDiscard} disabled={backConfirmSaving}>
                  No, Discard Changes
                </AlertDialogCancel>
                <AlertDialogAction onClick={handleBackConfirmSave} disabled={backConfirmSaving}>
                  {backConfirmSaving
                    ? currentVersionIndex === null
                      ? "Saving…"
                      : "Updating…"
          ...
          {currentVersionIndex !== null && (
            <p className="text-center text-xs text-muted-foreground mt-1">{autosaveStatusLabel(resumeAutosaveStatus)}</p>
          )}
          {selectedJob && (
  ...
  onOpenCoverLetterWizard={handleCreateCoverLetterFromResume}
  onOpenCoverLetterEditor={handleOpenCoverLetterEditorFromResume}
  hasSavedCoverLetter={!!currentResumeCoverLetter}
            />
          </div>
        </div>
      </div>
